...
module ActionView
  module Helpers
        ...
        unless count.zero?
          ...
          I18n.with_options :locale => options[:locale], :scope => [:activerecord, :errors, :template] do |locale|
            ...
            error_messages = objects.sum do |object|
            	errors = sort_by_manual(object)
              if errors.is_a?(ActiveModel::Errors)
	              errors.full_messages.map do |msg|
	                content_tag(:li, msg)
	              end
	            else
	            	errors.flatten.map do |msg|
	            		content_tag(:li, msg)
	            	end
	            end
            end.join.html_safe
    ...
    private
    	...
    	def sort_by_manual(obj)
    		errors = []
    		ori_errors = obj.errors
    		flag = true
    		obj.class::HUMAN_LIZE_ORDER rescue flag = false
    		if flag && obj.class::HUMAN_LIZE_ORDER.present? && obj.class::HUMAN_LIZE_ORDER.is_a?(Hash)
    			columns = obj.class::HUMAN_LIZE_ORDER.sort_by{|key, value| value}.collect{|c| c[0]}
    			columns.delete_if{|c| !ori_errors.keys.include?(c)}
    			columns.each do |key|
    				messages = []
    			  ori_errors[key.to_sym].each do |error|
    			  	head = I18n.t(:"activerecord.attributes.#{obj.class.to_s.downcase}.#{key.to_s.downcase}")
    			  	if head.include?("missing")
    			  		head = key.to_s
    			  	end
    					messages << head + error
    				end
